...
Sing-Box Windows 是一个围绕 sing-box 内核构建的现代桌面客户端，目标是把订阅导入、代理模式切换、运行状态查看和桌面集成整合成一套更顺手的跨平台体验。当前版本在设置页新增了可选的托盘轻量模式：关闭主窗口时可以直接销毁窗口实例，仅保留托盘与后台任务，以进一步降低后台 `WebView` 占用。
...
- **桌面集成**：支持托盘、开机启动、开机隐藏到托盘、轻量模式（关闭时销毁主窗口）、更新检查、窗口状态记忆。
- **可靠性能力**：提供备份恢复、订阅回滚、更新通道与内核自愈重启能力。
- **多语言**：内置 `zh-CN`、`en-US`、`ja-JP`、`ru-RU`。

### 托盘轻量模式

- **入口**：`设置 -> 启动设置 -> 关闭窗口行为`，可在“隐藏到托盘”和“轻量模式（销毁窗口）”之间切换。
- **行为**：启用后点击关闭按钮不会退出应用，而是销毁主窗口，仅保留托盘、后台内核与代理状态；托盘中的系统代理、TUN、重启内核等操作仍可继续使用。
- **恢复**：从托盘重新打开时会重建主窗口，并恢复到关闭前的最近页面。
- **联动**：如果同时开启“开机后自动隐藏到托盘”，系统自启动进入后台时也会继承轻量模式策略。
